...
if ( ! function_exists( 'cntctfrmtdb_action_links' ) ) {
	function cntctfrmtdb_action_links() {
		...
		if ( ( isset( $_REQUEST['action'] ) || isset( $_REQUEST['action2'] ) ) && check_admin_referer( plugin_basename( __FILE__ ), 'cntctfrmtdb_manager_nonce_name' ) ) {
			...
			$prefix = $wpdb->prefix . 'cntctfrmtdb_';
			$action = ( isset( $_REQUEST['action'] ) && '-1' != sanitize_text_field( wp_unslash( $_REQUEST['action'] ) ) ) ? $_REQUEST['action'] : $_REQUEST['action2'];
						...
						switch ( $action ) {
							...
							case 'restore':
								if ( isset( $_REQUEST['old_status'] ) && '' != sanitize_text_field( wp_unslash( $_REQUEST['old_status'] ) ) ) {
									$wpdb->update( $prefix . 'message', array( 'status_id' => absint( $_REQUEST['old_status'] ) ), array( 'id' => $id ) );
								} else {
...
if ( ! function_exists( 'cntctfrmtdb_number_of_messages' ) ) {
	function cntctfrmtdb_number_of_messages() {
		...
		if ( isset( $_REQUEST['s'] ) && $_REQUEST['s'] ) {
			$search     = sanitize_text_field( wp_unslash( trim( $_REQUEST['s'] ) ) );
			...
			<?php if ( ! $bws_hide_premium_options_check ) { ?>
				<div class="bws_pro_version_bloc">
					...
					<div class="bws_pro_version_tooltip">
						<a class="bws_button" href="https://bestwebsoft.com/products/wordpress/plugins/contact-form-to-db/?k=5906020043c50e2eab1528d63b126791&pn=91&v=<?php echo $cntctfrmtdb_plugin_info['Version']; ?>&wp_v=<?php echo $wp_version; ?>" target="_blank" title="Contact Form to DB Pro"><?php esc_html_e( 'Upgrade to Pro', 'contact-form-to-db' ); ?></a>
						<div class="clear"></div>
					</div>
				</div>
			<?php } else { ?>
				<p>
					<?php
					_e( 'This tab contains Pro options only.', 'contact-form-to-db' );
					echo ' ' . sprintf(
						__( '%1$sChange the settings%2$s to view the Pro options.', 'contact-form-to-db' ),
...
if ( ! function_exists( 'cntctfrmtdb_manager_page' ) ) {
	function cntctfrmtdb_manager_page() {
		...
		<div class="wrap cntctfrmtdb">
			...
			?>
			><p><?php echo $cntctfrmtdb_done_message; ?></p></div>
			<div class="error below-h2" 
			<?php
			if ( '' == $cntctfrmtdb_error_message ) {
				echo 'style="display: none;"';}
			?>
			><p><strong><?php esc_html_e( 'WARNING:', 'contact-form-to-db' ); ?></strong> <?php echo $cntctfrmtdb_error_message . ' ' . __( 'Please, try it later.', 'contact-form-to-db' ); ?></p></div>
			<?php
			if ( isset( $_REQUEST['s'] ) && $_REQUEST['s'] ) {
					printf( '<span class="subtitle">' . sprintf( __( 'Search results for &#8220;%s&#8221;', 'contact-form-to-db' ), wp_html_excerpt( sanitize_text_field( wp_unslash( $_REQUEST['s'] ) ), 50 ) ) . '</span>' );}
			$cntctfrmtdb_manager->views();
			?>
			<form id="posts-filter" method="get">
				<input type="hidden" name="page" value="cntctfrmtdb_manager" />
				<input type="hidden" name="message_status" class="message_status_page" value="<?php echo ! empty( $_REQUEST['message_status'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['message_status'] ) ) : 'all'; ?>" />
				<?php
...
if ( ! function_exists( 'cntctfrmtdb_read_message' ) ) {
	function cntctfrmtdb_read_message() {
		...
		$wpdb->update( $prefix . 'message', array( 'was_read' => sanitize_text_field( wp_unslash( $_POST['cntctfrmtdb_ajax_read_status'] ) ) ), array( 'id' => absint( $_POST['cntctfrmtdb_ajax_message_id'] ) ) );
		die();
	}
}
...
if ( ! function_exists( 'cntctfrmtdb_change_status' ) ) {
	function cntctfrmtdb_change_status() {
		...
		$prefix = $wpdb->prefix . 'cntctfrmtdb_';
		check_ajax_referer( plugin_basename( __FILE__ ), 'cntctfrmtdb_ajax_nonce_field' );
		$wpdb->update( $prefix . 'message', array( 'status_id' => sanitize_text_field( wp_unslash( $_POST['cntctfrmtdb_ajax_message_status'] ) ) ), array( 'id' => absint( $_POST['cntctfrmtdb_ajax_message_id'] ) ) );
		if ( ! $wpdb->last_error ) {
...
if ( ! function_exists( 'cntctfrmtdb_show_notices' ) ) {
	function cntctfrmtdb_show_notices() {
		...
		if ( $hook_suffix == 'plugins.php' || ( isset( $_REQUEST['page'] ) && in_array( sanitize_text_field( wp_unslash( $_REQUEST['page'] ) ), $cntctfrmtdb_pages ) ) ) {
			...
			if ( ! empty( $contact_form_notice ) ) {
				...
				<div class="error notice">
					<p><strong><?php esc_html_e( 'WARNING:', 'contact-form-to-db' ); ?></strong> <?php echo $contact_form_notice; ?></p>
				</div>
				<?php
			}
		}
